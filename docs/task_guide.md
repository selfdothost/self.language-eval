...
These YAML configuration files, along with the current codebase commit hash, are intended to be shareable such that providing the YAML config enables another researcher to precisely replicate the evaluation setup used by another, in the case that the prompt or setup differs from standard `language-eval` task implementations.
...
A full list of supported filter operations can be found in `language_eval/filters/__init__.py`. Contributions of new filter types are welcome!
...
```python
from language_eval.api.filter import Filter
from language_eval.api.registry import register_filter
...
Here we list all metrics currently supported natively in `language-eval`:
...
The default metric and aggregation functions are in `language_eval/api/metrics.py`, and you can add a function there if it's for general use. The metrics are towards the bottom of the file and look like this:
...
This function returns a single numeric value. The input is defined in `Task.process_results` in `language_eval/api/task.py`. There's a section that looks like this:
...
- SciQ (`language_eval/tasks/sciq/sciq.yaml`)

Corpus perplexity evaluations:

- Wikitext (`language_eval/tasks/wikitext/wikitext.yaml`)

Generative tasks:

- GSM8k (`language_eval/tasks/gsm8k/gsm8k.yaml`)

Tasks using complex filtering:

- GSM8k with CoT (+ with Self-Consistency): (`language_eval/tasks/gsm8k/gsm8k-cot.yaml` ; `language_eval/tasks/gsm8k/gsm8k-cot-self-consistency.yaml`)
