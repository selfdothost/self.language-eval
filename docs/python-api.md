...
results = language_eval.simple_evaluate(
    model="hf",
...
See [`language_eval/evaluator.py`](../language_eval/evaluator.py) for the complete parameter list.
...
```python
import language_eval
from language_eval.tasks import TaskManager, get_task_dict
from language_eval.models.huggingface import HFLM
...
# Run evaluation
results = language_eval.evaluate(
    lm=lm,
    task_dict=task_dict,
    num_fewshot=5,
    limit=100,
...
```python
from language_eval.tasks import get_task_dict
...
To evaluate a custom model, create a subclass of `language_eval.api.model.LM`:
...
```python
from language_eval.utils import setup_logging
...
import json
import language_eval
from language_eval.utils import handle_non_serializable
