...
This document details the interface exposed by `language-eval` and provides details on what flags are available to users.

## Command-line Interface

The `language-eval` CLI is organized into subcommands:

| Command | Description |
|---------|-------------|
| `language-eval run` | Run evaluations on language models |
| `language-eval ls` | List available tasks, groups, subtasks, or tags |
| `language-eval validate` | Validate task configurations |

Run the library via the `language-eval` entrypoint or `python -m language_eval`.

Use `-h` or `--help` to see available options:

```bash
language-eval -h              # Show all subcommands
language-eval run -h          # Show options for run command
language-eval ls -h           # Show options for list command
```

> **Legacy Compatibility**: The original single-command interface still works. Running `language-eval --model hf --tasks hellaswag` automatically inserts the `run` subcommand.
...
# Basic evaluation
language-eval run --model hf --model_args pretrained=gpt2 --tasks hellaswag

# With few-shot examples
language-eval run --model hf --model_args pretrained=gpt2 --tasks arc_easy --num_fewshot 5

# Save results and model outputs
language-eval run --model hf --model_args pretrained=gpt2 --tasks hellaswag --output_path ./results/ --log_samples

# Use a config file
language-eval run --config eval_config.yaml
```
...
```bash
language-eval run --model <model> --tasks <task> [options]
```
...
language-eval run --model hf --model_args pretrained=gpt2 dtype=float32 --tasks hellaswag

# Multiple tasks with few-shot examples
language-eval run --model vllm --model_args pretrained=EleutherAI/gpt-j-6B --tasks arc_easy arc_challenge --num_fewshot 5

# Custom generation parameters
language-eval run --model hf --model_args pretrained=gpt2 --tasks lambada --gen_kwargs temperature=0.8 top_p=0.95

# Use a YAML configuration file
language-eval run --config my_config.yaml --tasks mmlu
```
...
| `--tasks` | `-t` | Space or comma-separated list of task names or groups. Use `language-eval ls tasks` to see available tasks. |
...
|----------|-------------|
| `--cache_requests` | Cache preprocessed prompts: `true`, `refresh`, or `delete`. Cached files stored in `language_eval/cache/.cache` or path set by `LM_HARNESS_CACHE_PATH` env var. |
...
```bash
language-eval ls [tasks|groups|subtasks|tags] [--include_path DIR]
```
...
language-eval ls groups

# Include external tasks
language-eval ls tasks --include_path /path/to/external/tasks
```

---

## `language-eval validate`

Validate task configurations before running evaluations.

```bash
language-eval validate --tasks <task1,task2> [--include_path DIR]
```
...
language-eval validate --tasks hellaswag

# Validate multiple tasks
language-eval validate --tasks arc_easy,arc_challenge,hellaswag

# Validate a task group
language-eval validate --tasks mmlu

# Validate external tasks
language-eval validate --tasks my_custom_task --include_path ./custom_tasks
```
...
| `LM_HARNESS_CACHE_PATH` | Path for cached requests (default: `language_eval/cache/.cache`). |
