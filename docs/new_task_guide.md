...
This documentation page provides a walkthrough to get started creating your own task, in `language-eval` versions v0.4.0 and later.
...
To implement a new standard task, we'll need to write a YAML file which configures our task logic. We start by making a new empty YAML file. This file can have any name, but we recommend placing it in a subfolder of `language_eval/tasks` titled by the dataset or task's shorthand name: for example,

```sh
touch language_eval/tasks/<dataset_name>/<my_new_task_name>.yaml
```

Or, copy the template subfolder we provide from `templates/new_yaml_task`:

```sh
cp -r templates/new_yaml_task language_eval/tasks/
```
...
See `language_eval/tasks/minerva_math/minerva_math_algebra.yaml` for an example of the latter, and `language_eval/tasks/gsm8k/gsm8k-cot.yaml` for an example of the former.
...
```bash
touch language_eval/tasks/<dataset_name>/utils.py
```

Now, in `utils.py` we'll write a function to process each split of our dataset (the following example is drawn from [the `hellaswag` task](../language_eval/tasks/hellaswag/utils.py)):
...
To test a task conveniently, it helps to *register* the task--that is, to give it a name and make the `language-eval` library aware it exists!

If you're writing your YAML file inside the `language_eval/tasks` folder, you just need to give your task a name! You can do this inside your YAML file:
...
If your task is not in the `language_eval/tasks` folder, you'll need to tell the Eval Harness where to look for YAML files.
...
It is recommended to include a filled-out copy of this checklist in the README.md for the subfolder you are creating, if you have created a new subfolder in `language_eval/tasks`.

**Finally, please add a short description of your task(s), along with a link to its subfolder in language_eval/tasks, to [`language_eval/tasks/README.md`](https://github.com/EleutherAI/lm-evaluation-harness/blob/main/lm_eval/tasks/README.md) so that users can discover your task in the library, and follow the link to your README for more information about the variants supported, their task names, and the original source of the dataset and/or evaluation setup.**
