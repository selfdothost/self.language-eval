...
This guide may be of special interest to users who are using the library outside of the repository, via installing the library via pypi and calling `language_eval.evaluator.evaluate()` to evaluate an existing model.

In order to properly evaluate a given LM, we require implementation of a wrapper class subclassing the `language_eval.api.model.LM` class, that defines how the Evaluation Harness should interface with your model. This guide walks through how to write this `LM` subclass via adding it to the library!
...
```sh
touch language_eval/models/<my_model_filename>.py
```
...
All models must subclass the `language_eval.api.model.LM` class.
...
Where `Instance` is a dataclass defined in [`language_eval.api.instance`](https://github.com/EleutherAI/lm-evaluation-harness/blob/main/lm_eval/api/instance.py) with property `args` of request-dependent type signature described below.
...
To allow a model to be evaluated on all types of tasks, you will need to implement these three types of measurements (note that `loglikelihood_rolling` is a special case of `loglikelihood`). For a reference implementation, check out `language_eval/models/huggingface.py` ! Additionally, check out `language_eval.api.model.TemplateLM` for a class that abstracts away some commonly used functions across LM subclasses, or see if your model would lend itself well to subclassing the `language_eval.models.huggingface.HFLM` class and overriding just the initialization or a couple methods!
...
To make your model usable via the command line interface to `language-eval` using `python -m language_eval`, you'll need to tell `language-eval` what your model's name is.

This is done via a *decorator*, `language_eval.api.registry.register_model`. Using `register_model()`, one can both tell the package what the model's name(s) to be used are when invoking it with `python -m language_eval --model <name>` and alert `language-eval` to the model's existence.
...
Using this decorator results in the class being added to an accounting of the usable LM types maintained internally to the library at `language_eval.api.registry.MODEL_REGISTRY`. See `language_eval.api.registry` for more detail on what sorts of registries and decorators exist in the library!

**Tip: be sure to import your model in `language_eval/models/__init__.py!`**
...
**Pro tip**: In order to make the Evaluation Harness overestimate total runtimes rather than underestimate it, HuggingFace models come in-built with the ability to provide responses on data points in *descending order by total input length* via `language_eval.utils.Reorderer`. Take a look at `language_eval.models.hf_causal.HFLM` to see how this is done, and see if you can implement it in your own model!
