...
This guide explains how to use YAML configuration files with `language-eval` to define reusable evaluation settings.
...
```bash
# Instead of:
language-eval run --model hf --model_args pretrained=gpt2,dtype=float32 --tasks hellaswag arc_easy --num_fewshot 5 --batch_size 8 --device cuda:0
...
```bash
language-eval run --config eval_config.yaml --tasks mmlu --limit 100
```

## Quick Reference

All configuration keys correspond directly to CLI arguments. See the [CLI Reference](interface.md#language-eval-run) for detailed descriptions of each option.
...
language-eval validate --tasks hellaswag,arc_easy

# With external tasks
language-eval validate --tasks my_task --include_path /path/to/tasks
```
