...
Contamination detection can be found in `language_eval/decontaminate.py` with supporting code in `language_eval/decontamination/`.
...
`language_eval/evaluator.py` can then produce a clean version of the benchmark by excluding the results of contaminated documents. For each metric, a clean version will be shown in the results with a "decontaminate" suffix.
