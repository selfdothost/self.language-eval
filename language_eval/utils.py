...
class _LMEvalFormatter(logging.Formatter):
    """Formatter that strips 'language_eval.' prefix from logger names for cleaner output."""

    def format(self, record):
        record.short_name = record.name.removeprefix("language_eval.")
...
def setup_logging(verbosity=logging.INFO):
    """Configure logging for language_eval.
    ...
    language_eval_logger = logging.getLogger("language_eval")
    language_eval_logger.setLevel(log_level)
    ...
    if not _LOGGING_CONFIGURED:
        ...
        language_eval_logger.addHandler(handler)

        # Don't propagate to root to avoid duplicate logs if root is also configured
        language_eval_logger.propagate = False
