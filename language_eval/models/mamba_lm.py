...
class MambaLMWrapper(HFLM):
    ...
    please install mamba via `pip install language-eval[mamba]` or `pip install -e .[mamba]`",
                ) from exception
    ...
    please install mamba via `pip install language-eval[mamba]` or `pip install -e .[mamba]`",
            ...
            self._model = MambaLMHeadModel.from_pretrained(
                ...
                dtype=torch.float16
                if dtype == "auto"
                else language_eval.models.utils_hf.get_dtype(dtype),
            )
    ...
    def _model_generate(self, context, max_length, stop, **generation_kwargs):
        ...
        if not self.is_hf:
            ...
            stopping_criteria = language_eval.models.utils_hf.stop_sequences_criteria(
                self.tokenizer,
                stop,
                context.shape[1],
                context.shape[0],
