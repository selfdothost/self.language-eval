...
Usage Examples:
    ...
    torchrun --nproc_per_node=1 -m language_eval --model megatron_lm \
    ...
    torchrun --nproc_per_node=4 -m language_eval --model megatron_lm \
        --model_args load=/path/to/ckpt,devices=4,tokenizer_model=/path/to/tokenizer.model \
        --tasks arc_easy --batch_size 8

    # Tensor Parallelism (2 GPUs for TP)
    torchrun --nproc_per_node=2 -m language_eval --model megatron_lm \
    ...
    torchrun --nproc_per_node=6 -m language_eval --model megatron_lm \
...
from language_eval.api.instance import Instance
from language_eval.api.model import LM
from language_eval.api.registry import register_model
from language_eval.models.utils import Collator
from language_eval.utils import get_rolling_token_windows, make_disjoint_window
...
class MegatronLMEval(LM):
    ...
    def _initialize_megatron(self, **kwargs):
        ...
        try:
            ...
            if self._parallelism_mode == "data_parallel":
                ...
                # From language_eval's perspective, this is a single worker (world_size=1)
    ...
    def _distribute_requests(self, requests: list) -> tuple[list, list[int]]:
        ...
        NOTE: When world_size > 1, language_eval's evaluator already distributes
        ...
        """
        # language_eval already handles data distribution when world_size > 1
    ...
    def _gather_results(self, local_results: list, sizes: list[int]) -> list:
        ...
        NOTE: When world_size > 1, language_eval's evaluator already handles
        ...
        """
        # language_eval already handles result gathering when world_size > 1
    ...
    def _loglikelihood_tokens(
        ...
        - Requests are distributed across ranks by language_eval's evaluator
        - Each rank processes its share
        - Results are gathered by language_eval's evaluator
        ...
        - language_eval's evaluator ensures equal request counts across ranks
        """
    ...
    def generate_until(
        ...
        for chunk in chunks:
            ...
            # (in practice, language_eval usually uses same params for all requests)
