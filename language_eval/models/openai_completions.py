...
from language_eval.models.api_models import TemplateAPI
from language_eval.models.utils import handle_stop_sequences
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if tokenizer_backend == "auto":
            if base_url:
                from language_eval.utils import check_remote_tokenizer_support
...
class AzureOpenaiChatCompletionsLM(OpenAIChatCompletion):
    ...
    please install these via `pip install language-eval[openai]` or `pip install -e .[openai]`",
            )
        self.model = model
