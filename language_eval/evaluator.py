...
import language_eval.api.model
import language_eval.api.registry
from language_eval.caching.cache import delete_cache
from language_eval.defaults import DEFAULT_OTHER_SEED, DEFAULT_RANDOM_SEED, LMEVAL_HASHMM
from language_eval.evaluator_utils import (
    ResultAcc,
    _handle_back_comp,
    _log_selected_tasks,
    _process_results,
...
from language_eval.loggers.utils import add_env_info, add_tokenizer_info, get_git_commit_hash
from language_eval.tasks import TaskManager
from language_eval.utils import (
...
if TYPE_CHECKING:
    from language_eval.api.group import Group
    from language_eval.api.model import LM
    from language_eval.api.task import Task
    from language_eval.loggers import EvaluationTracker
    from language_eval.result_schema import EvalResults
    from language_eval.tasks.manager import TaskDict
...
def simple_evaluate(
    ...
    Args:
        model (str | LM): Name of model or LM object. See
            language_eval.models.__init__.py for available aliases.
    ...
    if isinstance(model, str):
        ...
        if isinstance(model_args, dict):
            ...
            lm = language_eval.api.registry.get_model(model).create_from_arg_obj(
                model_args,
                {
            ...
            lm = language_eval.api.registry.get_model(model).create_from_arg_string(
                model_args,
                {
                    "batch_size": batch_size,
                    "max_batch_size": max_batch_size,
        ...
        if not isinstance(model, language_eval.api.model.LM):
            raise TypeError(
                f"The value of `model` passed to simple_evaluate() was of type {type(model)}, but is required to be a subclass of language_eval.api.model.LM . This may be because you are passing an initialized Hugging Face PreTrainedModel without having wrapped it in `language_eval.models.huggingface.HFLM(pretrained=my_model)` first."
            )
    ...
    if use_cache is not None:
        ...
        lm = language_eval.api.model.CachingLM(
            lm,
            use_cache
