"""Registry system for language_eval components.

This module provides a centralized registration system for models, tasks, metrics,
filters, and other components in the language_eval framework.
...
def get_model(model_name: str):
    ...
    if len(model_registry) == 0:
        import language_eval.models  # noqa: F401
...
def get_metric(name: str, hf_evaluate_metric: bool = False) -> Callable | None:
    ...
    if len(metric_registry) == 0:
        import language_eval.api.metrics  # noqa: F401
    ...
    if not hf_evaluate_metric:
        if name in metric_registry:
            ...
            eval_logger.warning(
                f"Could not find registered metric '{name}' in language-eval, searching in HF Evaluate library..."
            )
