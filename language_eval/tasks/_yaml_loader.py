...
def _load_module_with_cache(module_path: Path) -> Any:
    ...
    if "/language_eval/tasks/" in path_str:
        # Find the position of language_eval/tasks/ in the path
        tasks_idx = path_str.find("/language_eval/tasks/")
        if tasks_idx != -1:
            # Extract path starting from language_eval/tasks/
            # e.g., /path/to/language_eval/tasks/hellaswag/utils.py → hellaswag/utils.py
            relative_path = path_str[tasks_idx + len("/language_eval/tasks/") :]
            # Remove .py and convert to module name
            # e.g., hellaswag/utils.py → language_eval.tasks.hellaswag.utils
            module_parts = relative_path.replace(".py", "").replace("/", ".")
            module_name = f"language_eval.tasks.{module_parts}"
        else:
