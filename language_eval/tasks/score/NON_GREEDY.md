...
3. When all 5 runs are finished and logs are saved, run the `./language_eval/tasks/score/non_greedy_summarizer.py` script by passing the the output directory of the above runs to the `--log_dir` argument***, and by specifying the dataset name for which the evaluations were run with `--dataset` argument(`agieval`, `mmlu_pro` or `math`). \

4. The script will return the default language_evaluation_harness table where accuracies for each seed and the consistancy rate are calculated.
...
\*\* _Refer to [`./language_eval/tasks/score/non_greedy.sh`](https://github.com/EleutherAI/lm-evaluation-harness/blob/main/lm_eval/tasks/score/non_greedy.sh) to see an example of non greedy evaluation command for each seed._
