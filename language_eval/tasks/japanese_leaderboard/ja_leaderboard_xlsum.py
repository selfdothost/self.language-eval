...
def _missing_module_message(name):
    return f"`{name}` is required for `japanese_leaderboard`, please install `{name}` via pip install language_eval[japanese_leaderboard] or pip install -e .[japanese_leaderboard]"
