...
from language_eval._cli.harness import HarnessCLI
from language_eval._cli.ls import List
from language_eval._cli.run import Run
...
from language_eval._cli.validate import Validate
...
class TestHarnessCLI:
    ...
    def test_harness_cli_backward_compatibility(self):
        ...
        cli = HarnessCLI()
        test_args = ["language-eval", "--model", "hf", "--tasks", "hellaswag"]
    ...
    def test_harness_cli_help_default(self):
        ...
        cli = HarnessCLI()
        with patch.object(sys, "argv", ["language-eval"]):
            args = cli.parse_args()
    ...
    def test_harness_cli_run_help_only(self):
        """Test that 'language-eval run' shows help."""
        cli = HarnessCLI()
        with patch.object(sys, "argv", ["language-eval", "run"]), pytest.raises(SystemExit):
            cli.parse_args()
...
class TestListCommand:
    ...
    @patch("language_eval.tasks.TaskManager")
...
class TestRunCommand:
    ...
    @patch("language_eval.simple_evaluate")
    @patch("language_eval.config.evaluate_config.EvaluatorConfig")
    @patch("language_eval.loggers.EvaluationTracker")
    @patch("language_eval.utils.make_table")
