...
from language_eval.models.openai_completions import LocalCompletionsAPI
...
def test_get_batched_requests_with_no_ssl(
    ...
    with (
        patch(
            "language_eval.models.api_models.TCPConnector", autospec=True
        ) as mock_connector,
        patch(
            "language_eval.models.api_models.ClientSession", autospec=True
        ) as mock_client_session,
        patch(
            "language_eval.models.openai_completions.LocalCompletionsAPI.parse_logprobs",
            autospec=True,
        ) as mock_parse,
    ):
...
def test_local_completionsapi_remote_tokenizer_authenticated(monkeypatch):
    ...
    monkeypatch.setattr("language_eval.utils.RemoteTokenizer", DummyTokenizer)
    LocalCompletionsAPI(
...
def test_localchatcompletion_remote_tokenizer_authenticated(monkeypatch):
    ...
    monkeypatch.setattr("language_eval.utils.RemoteTokenizer", DummyTokenizer)
    from language_eval.models.openai_completions import LocalChatCompletion
