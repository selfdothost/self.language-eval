...
> **self.language-eval** — the catch-all harness for **language-focused**
> evaluation tasks in the self.ai stack. It is a fully vendored, GPLv3-relicensed
> fork of EleutherAI's [Language Model Evaluation Harness](https://github.com/EleutherAI/lm-evaluation-harness)
> (MIT), previously carried as `self.lm-evaluation-harness`; the Python package is
> `language_eval`. Not affiliated with or endorsed by EleutherAI. See
> [`NOTICE`](NOTICE) for provenance and [`LICENSE`](LICENSE) (GPLv3); the original
> MIT text is retained in `LICENSE.eleutherai-mit`.
...
```bash
pip install "language_eval[hf]"
```

For vLLM inference:

```bash
pip install "language_eval[vllm]"
```

For API-based models (OpenAI, Anthropic, etc.):

```bash
pip install "language_eval[api]"
```

Multiple backends can be installed together:

```bash
pip install "language_eval[hf,vllm,api]"
```
...
| [Task Guide](./language_eval/tasks/README.md) | Available tasks and task configuration |

Use `language-eval -h` to see available options, or `language-eval run -h` for evaluation options.

List available tasks with:

```bash
language-eval ls tasks
```

### Hugging Face `transformers`

> [!Important]
> To use the HuggingFace backend, first install: `pip install "language_eval[hf]"`

To evaluate a model hosted on the [HuggingFace Hub](https://huggingface.co/models) (e.g. GPT-J-6B) on `hellaswag` you can use the following command (this assumes you are using a CUDA-compatible GPU):

```bash
language_eval --model hf \
...
> [!Note]
> Just like you can provide a local path to `transformers.AutoModel`, you can also provide a local path to `language_eval` via `--model_args pretrained=/path/to/model`

#### Evaluating GGUF Models

`language-eval` supports evaluating models in GGUF format using the Hugging Face (`hf`) backend. This allows you to use quantized models compatible with `transformers`, `AutoModel`, and llama.cpp conversions.
...
```bash
accelerate launch -m language_eval --model hf \
    --tasks lambada_openai,arc_easy \
    --batch_size 16
```

(or via `accelerate launch --no-python language_eval`).
...
```bash
torchrun --nproc-per-node=8 --no-python language_eval \
    --model nemo_lm \
...
```bash
torchrun --nproc-per-node=4 --no-python language_eval \
    --model nemo_lm \
...
```bash
language_eval --model megatron_lm \
...
```bash
torchrun --nproc-per-node=4 -m language_eval --model megatron_lm \
...
```bash
torchrun --nproc-per-node=2 -m language_eval --model megatron_lm \
...
> [!Important]
> To use API-based models, first install: `pip install "language_eval[api]"`
...
language_eval --model openai-completions \
...
```bash
language_eval --model local-completions --tasks gsm8k --model_args model=facebook/opt-125m,base_url=http://{yourip}:8000/v1/completions,num_concurrent=1,max_retries=3,tokenized_requests=False,batch_size=16
```
...
language_eval --model hf \
    ...
    --log_samples \
    --output_path results \
    --hf_hub_log_args hub_results_org=EleutherAI,hub_repo_name=language-eval-results,push_results_to_hub=True,push_samples_to_hub=True,public_repo=False \
```
...
You'll also need to install the `language_eval[zeno]` package extra.
...
First you'll need to install the language_eval[wandb] package extra. Do `pip install language_eval[wandb]`.
...
language_eval \
    ...
    --limit 10 \
    --wandb_args project=language-eval-harness-integration \
    --log_samples
```
